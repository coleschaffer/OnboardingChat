...
/**
 * Copyable WhatsApp message block (custom message)
 */
function createWhatsAppCopyBlock(message, baseUrl = null, title = '*💬 WhatsApp Message:*') {
    const safeMessage = message || '';
    const copyUrl = `${baseUrl || process.env.BASE_URL || 'https://onboarding.copyaccelerator.com'}/copy.html?text=${encodeURIComponent(safeMessage)}`;
 ...
 */
function createCancellationReasonBlock(memberLabel, reason, createdBy = null) {
    const byLine = createdBy ? `\n_Added by ${createdBy}_` : '';
    return {
        blocks: [
            {
                type: 'section',
                text: {
                    type: 'mrkdwn',
                    text: `*🛑 Cancellation Reason for ${memberLabel}*`
                }
            },
            {
                type: 'section',
...
function stripQuotedContent(text) {
    ...
    // Pattern 1: Gmail style - "On ... wrote:" possibly split across lines
    // Match "On" at start of line, followed by date/time/name/email, ending with "wrote:"
    // This handles cases where Gmail wraps the line before "wrote:"
    result = result.replace(/\r?\n\r?\nOn [^\n]*\r?\n?[^\n]*wrote:\r?\n?[\s\S]*/i, '');

    // Pattern 2: Single-line Gmail style (simpler case)
    result = result.replace(/\r?\n\r?\nOn .+wrote:\r?\n?[\s\S]*/i, '');
    ...
    // Pattern 6: Gmail mobile style "On [date] <[email]> wrote:"
    result = result.replace(/\r?\n\r?\nOn .+<[^>]+>\s*wrote:\r?\n?[\s\S]*/i, '');
 ...
 */
function createEmailReplyBlock({
    recipientName,
    recipientEmail,
    replySnippet,
    replyBody,
 ...
 */
function createCallBookedBlock(applicantName, applicantEmail, eventTime, stefanSlackId, meetingNotes = '') {
    const blocks = [
        {
            type: 'section',
            text: {
 ...
 */
function createSendMessageModal(recipientName, recipientEmail, threadId, contextType = 'typeform_application', contextId = null, previousText = '') {
    return {
        type: 'modal',
...
function createNoteAddedBlock(noteText, createdBy, createdAt) {
    const timezone = process.env.DEFAULT_TIMEZONE || DEFAULT_TIMEZONE;
    const dateValue = createdAt ? new Date(createdAt) : new Date();
    const timestamp = dateValue.toLocaleString('en-US', {
        timeZone: timezone,
        month: 'short',
        day: 'numeric',
        hour: 'numeric',
...
module.exports = {
    ...
    createWhatsAppCopyBlock,
    createEmailFailedBlock,
    createEmailSentBlock,
    createCancellationReasonBlock,
